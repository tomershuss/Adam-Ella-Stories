...
ELEVENLABS_STABILITY = 0.5                        # optional; 0.0 = most expressive, 0.5 = natural, 1.0 = flat.
```
...
2. **Tone for the narration.** The recorded voice follows direction tags that never appear on screen: `data-tone="…"` on `<main>` sets the default for the whole story (keep `gentle, slow` unless the story needs otherwise), and any sentence can override it with its own `data-tone="…"` (comma-separated English words, 1–3 of them, e.g. `whispers, slow` for the refrain and the good-night lines, `curious`, `surprised, playful`, `warm`, `happy`, `sleepy, very soft` for the ending, `sad, soft` for a small disappointment). `data-tone=""` on a sentence means no tag. Use overrides sparingly, on 5–10 sentences per story where the emotion clearly changes; the default carries the rest. Never write tone words into the visible text.
3. 6–10 story pages: `<section class="page" data-kind="story">` each with `<figure class="art"><svg viewBox="0 0 300 400" role="img" aria-label="…">…</svg></figure>` (portrait 3:4, a full scene, characters big) and `<div class="text">` with 2–5 sentences, each in its own `<p class="s">`; the refrain in `<p class="s refrain">`. The first page has `<h1>` with the title and the button `<button class="textbtn" id="readAllBtn" type="button">…</button>` exactly as in the template (label text in the story language). Total story-page words: **600–900**. Structure: character → small problem → discovery → calm ending. The last story page is quiet and sleepy.
4. Music page (`data-kind="music"`) only if Step 3 verified a video: heading "בואו נשמע: …" / "Let's listen: …", 2–3 sentences about the piece, the `music-box` with `data-video="<id>"` on `#playBtn`, the `<iframe id="musicFrame">` left without `src` (the engine loads `youtube-nocookie.com/embed/<id>?rel=0` on press), and the plain YouTube link below it. Otherwise delete the section.
5. "Before we sleep" page (`data-kind="before-sleep"`): a quiet illustration, `<h2>`, an `<ol class="qa">` with **2 questions** for the parent to ask (`<li class="s">`, e.g. "What would you do instead of …?"), and one `<p class="s"><b>מחר תוכלו</b> …</p>` / `<b>Tomorrow you can</b> …`: a 2-minute activity at home connected to the story.
6. "Good night" page (`data-kind="goodnight"`): a very quiet illustration, `<h1 class="s">לילה טוב</h1>` / `Good night`, one soft sentence, then `<details>` "למבוגרים: מה נכון פה ומה המקורות" / "For grown-ups: what is true here, and sources" with 2–3 short paragraphs and the source links (for science/history/geography/art/music stories; for feelings stories a short note is enough), and the `<p class="meta">` line with the library link `/Adam-Ella-Stories/e/`, `סיפור מספר <span class="meta-n">N</span>` and `<span class="meta-date">YYYY-MM-DD</span>`.
7. Every `<svg>` in a page gets an `aria-label` describing the scene in the story language. Gentle animation is welcome through the existing classes `twinkle`, `drift`, `sea`, `pull`; nothing flashing.
8. No network requests except Google Fonts and YouTube. No external images, scripts or styles.
...
2. Record, with the keys from the **Keys** section passed only as environment variables on the command line:
   ```bash
   ELEVENLABS_API_KEY='<key>' ELEVENLABS_VOICE_ID='<voice id>' ELEVENLABS_MODEL='eleven_v3' ELEVENLABS_STABILITY='0.5' node scripts/make-audio.mjs --all
   ```
